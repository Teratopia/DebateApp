...
(function(angular) {
	...
	angular
			...
			.animation(
					...
					[
							'$animateCss',
							'accordionConfig',
							function($animateCss, accordionConfig) {
								return {
									addClass : function(element, className,
											done) {
										var paneContent = angular
												.element(element[0]
														.querySelector('v-pane-content')), paneInner = angular
												.element(paneContent[0]
														.querySelector('div'));

										var height = paneInner[0].offsetHeight;

										var expandAnimation = $animateCss(
												paneContent,
												{
													easing : 'ease',
													from : {
									...
									removeClass : function(element, className,
										...
										var collapseAnimation = $animateCss(
												paneContent,
												{
													easing : 'ease',
													from : {
	...
	angular.module('vAccordion',
			[ 'vAccordion.config', 'vAccordion.directives' ]);

	// vAccordion directive
	angular.module('vAccordion.directives').directive('vAccordion',
			vAccordionDirective);
			...
			link : {
				...
				post : function(scope, iElement, iAttrs, ctrl, transclude) {
					...
					if (angular.isDefined(scope.control)) {
						...
						var mergedControl = angular.extend({},
					...
					function checkCustomControlAPIMethods() {
						var protectedApiMethods = [ 'toggle', 'expand',
								'collapse', 'expandAll', 'collapseAll',
								'hasExpandedPane' ];

						angular.forEach(protectedApiMethods, function(
								iteratedMethodName) {
							if (scope.control[iteratedMethodName]) {
								throw new Error('The `' + iteratedMethodName
										+ '` method can not be overwritten');
							}
						});
					}

					$timeout(function() {
						var eventName = (angular.isDefined(ctrl
								.getAccordionId())) ? ctrl.getAccordionId()
								+ ':onReady' : 'vAccordion:onReady';
	...
	function vAccordionController($scope) {
		...
		$scope.expandCb = (angular.isFunction($scope.expandCb)) ? $scope.expandCb
				: angular.noop;
		$scope.collapseCb = (angular.isFunction($scope.collapseCb)) ? $scope.collapseCb
				: angular.noop;
		...
		ctrl.getPaneByIndex = function getPaneByIndex(index) {
			...
			angular.forEach($scope.panes, function(iteratedPane) {
				if (iteratedPane.$parent
						&& angular.isDefined(iteratedPane.$parent.$index)
						&& iteratedPane.$parent.$index === index) {
		...
		ctrl.getPaneIndex = function getPaneIndex(pane) {
			...
			return (angular.isDefined(theIndex)) ? theIndex : $scope.panes
					.indexOf(pane);
		};
		...
		ctrl.toggle = function toggle(paneToToggle) {
			...
			if (paneToToggle.isExpanded) {
				$scope.expandCb({
					index : ctrl.getPaneIndex(paneToToggle),
					id : paneToToggle.id,
					pane : paneToToggle
				});
			} else {
		...
		ctrl.expand = function expand(paneToExpand) {
			...
			if (!paneToExpand.isExpanded) {
				...
				$scope.expandCb({
					index : ctrl.getPaneIndex(paneToExpand),
					id : paneToExpand.id,
					pane : paneToExpand
				});
			}
	...
	function vPaneDirective($timeout, $animate, accordionConfig) {
		return {
			...
			link : function(scope, iElement, iAttrs, accordionCtrl, transclude) {
				...
				var paneHeader = iElement.find('v-pane-header'), paneContent = iElement
						.find('v-pane-content'), paneInner = paneContent
						.find('div');
				...
				if (!paneContent[0]) {
					throw new Error(
							'The `v-pane-content` directive can\'t be found');
				}
				...
				function emitEvent(eventName) {
					eventName = (angular.isDefined(accordionId)) ? accordionId
							+ ':' + eventName : 'vAccordion:' + eventName;
					scope.$emit(eventName);
				}
				...
				scope.$evalAsync(function() {
					if (scope.isExpanded) {
						...
						paneContent.css('max-height', 'none').attr(
								'aria-hidden', 'false');
						...
						paneContent.css('max-height', '0px').attr(
								'aria-hidden', 'true');
	...
	angular.module('vAccordion.directives').directive('vPaneContent',
	...
	angular.module('vAccordion.directives').directive('vPaneHeader',
	...
	function vPaneHeaderDirective() {
		return {
			...
			link : function(scope, iElement, iAttrs, ctrls) {
				...
				var paneCtrl = ctrls[0], accordionCtrl = ctrls[1];
